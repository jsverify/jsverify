	...
	function forall() {
		...
		function test(size, x) {
			...
			return withPromise(r, function(r) {
								...
								if (shrinkP === true) {
									return withPromise(r_rec, function (r_rec) {
										...
										if (r_rec && r_rec.counterexample) {
											return {
												ounterexample: [x].concat(r_rec.counterexample),
												counterexamplestr: counterexamplestr + "; " + r_rec.counterexamplestr,
											};
										} else {
											return { counterexample: [x], counterexamplestr: counterexamplestr };
										}
									});
								} else {
									return shrinkP;
				...
				return withPromise(shrinkP, function (shrinkP) {
					if (shrinkP === true) {
						return { counterexample: [x], counterexamplestr: showManyWith(gens, x) };
					} else {
						return shrinkP;
					}
				});
