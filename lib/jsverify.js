	...
	*/
	function check(property, opts) {
		...
		function loop(i) {
			...
			var r = property(opts.size);
			return withPromise(r, function (r) {
				if (r === true) {
					return loop(i-1);
				} else {
					/* global console */
					if (!opts.quiet) { console.error("Propery doesn't hold, counterexample with", r.counterexample); }
					return r;
				}
			});
		}

		return withPromise(loop(opts.tests), function (r) {
			if (r === true) {
